...
use crate::{types::{CurrentCut, NewHead, BlockHeaderItems}, utils::{format_endpoint_with_query_params, req_header_content_type_with_accept}, BlockHeader};
...
impl Ingest {
  ...
  pub fn new(base_url: String, num_of_chains: u64, params: Option<QueryParams>) -> Self {
    ...
    for chain_id in 0..num_of_chains {
      ...
      headers_map.insert(chain_id, format!("{}/chain/{}{}", base_url, chain_id, endpoint));
    }
  ...
  pub async fn current_cut(&self) -> Result<CurrentCut, reqwest::Error> {
    let ccut = retry(ExponentialBackoff::default(), || async {
      ...
      Ok(reqwest::get(&self.cut_endpoint).await?.json::<CurrentCut>().await?)
    }).await?;
  ...
  pub async fn listen_new_heads(&self) -> Result<(), reqwest::Error> {
    ...
    while let Some(chunk) = res.chunk().await? {
        ...
        debug!("Received new head on chain {} at {}", _v.header.chain_id, _v.header.height);
  ...
  pub async fn blocks_headers(&self) -> Result<Vec<BlockHeader>, reqwest::Error> {
    ...
    for chain_id in chain_ids {
        ...
        let req_body = format!("{{\"upper\": [\"{}\"], \"lower\": []}}", hash_height.hash.clone());

        let h = tokio::spawn(async move {
          retry(ExponentialBackoff::default(), || {op(full_url.clone(), req_body.clone())}).await
        });
        handles.push(h);
    }
